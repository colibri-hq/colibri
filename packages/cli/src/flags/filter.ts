...
type ListFilter<K extends string, T extends FilterValue = FilterValue> = FilterObject<K, 'in' | 'not in', T[]>;
type StringFilter<K extends string> = FilterObject<K, '!=' | '!~' | '$~' | '=' | '^~' | '~', string>;
...
export const filterFactory = <T extends string = string>() => Flags.custom<Filter<T>>({
  char: 'f',
...
function parseFilter<K extends string>(
  ...
  if (
    (parsedOperator === 'is' || parsedOperator === 'is not') ||
    parsedValue === null
  ) {
