...
export function parseXmp(buffer: ArrayBufferLike | string) {
  ...
  return Object.entries(description ?? {})
    ...
    .reduce<XmpMetadata>((acc, [key, value]) => {
      const [namespace, property] = key.split(":", 2) as [XmpNamespaceName, string];
...
function isValidNamespace(name: string | XmpNamespaceName | undefined): name is XmpNamespaceName {
...
function parseGPSCoordinate(value: string) {
  ...
  return Number(a) / Number(b) + Number(c) / Number(d) / 60 + Number(e) / Number(f) / 3600;
}

type XmpBasePropertyValue = string | number | boolean | Date;
type XmpPropertyValue = XmpBasePropertyValue | XmpBasePropertyValue[];
type XmpNamespaceName = "dc" | "exif" | "exifEX" | "tiff" | "xmp" | "pdf" | "photoshop";
