...
describe.skipIf(!process.env.DATABASE_URL)("Ingestion Integration Tests", () => {
  ...
  beforeEach(async () => {
    ...
    await database.transaction().execute(async (trx) => {
      ...
      await trx.deleteFrom("asset").where("created_by", "=", TEST_USER_ID).execute();
      ...
      await trx
        .deleteFrom("work_tag")
        .where("work_id", "in", (eb) =>
          eb.selectFrom("work").select("id").where("created_by", "=", TEST_USER_ID),
        )
        .execute();
      ...
      // Delete works
      await trx.deleteFrom("work").where("created_by", "=", TEST_USER_ID).execute();

      // Clean up tags if they were created by test user
      await trx.deleteFrom("tag").where("created_by", "=", TEST_USER_ID).execute();
      ...
      await trx.deleteFrom("pending_ingestion").where("user_id", "=", TEST_USER_ID).execute();
    });
  });

  describe("Single File Ingestion", () => {
    it("should import new EPUB and create work/edition/asset", async () => {
      const options: IngestWorkOptions = { userId: TEST_USER_ID };
    ...
    it("should store asset with correct checksum", async () => {
      ...
      expect(Buffer.from(assetChecksum).equals(Buffer.from(testFileChecksum))).toBe(true);
    });
  });
  ...
  describe("Duplicate Detection", () => {
    ...
    it("should detect duplicate with onDuplicateWork='skip' option", async () => {
      // First import
      const firstResult = await ingestWork(database, testFile, { userId: TEST_USER_ID });
      expect(firstResult.status).toBe("created");
  ...
  describe("Creator/Publisher Normalization", () => {
    it("should create new creator on first import", async () => {
      const result = await ingestWork(database, testFile, { userId: TEST_USER_ID });
    ...
    it("should reuse existing creator with exact name match", async () => {
      ...
      await database
        ...
        .where("edition_id", "=", firstResult.edition!.id)
        .execute();
      await database.deleteFrom("edition").where("id", "=", firstResult.edition!.id).execute();
      await database.deleteFrom("work").where("id", "=", firstResult.work!.id).execute();

      // Second import should reuse the creator
      const secondResult = await ingestWork(database, testFile, { userId: TEST_USER_ID });
  ...
  describe("Series and Tags", () => {
    ...
    it("should not create duplicate tags when adding multiple works with same subjects", async () => {
      ...
      await database
        ...
        .where("edition_id", "=", firstResult.edition!.id)
        .execute();
      await database.deleteFrom("work_tag").where("work_id", "=", firstResult.work!.id).execute();
      await database.deleteFrom("edition").where("id", "=", firstResult.edition!.id).execute();
      await database.deleteFrom("work").where("id", "=", firstResult.work!.id).execute();
      ...
      const secondTags = await loadTagsForWork(database, secondResult.work!.id);
  ...
  describe("Pending Ingestion Workflow", () => {
    ...
    it("should retrieve and confirm pending ingestion with 'skip' action", async () => {
      ...
      const confirmResult = await confirmIngestion(database, pendingId, "skip");
    ...
    it("should retrieve and confirm pending ingestion with 'create-work' action", async () => {
      // Create pending ingestion
      await ingestWork(database, testFile, { userId: TEST_USER_ID });
      ...
      const confirmResult = await confirmIngestion(database, pendingId, "create-work", testFile);
    ...
    it("should retrieve and confirm pending ingestion with 'create-edition' action", async () => {
      ...
      const confirmResult = await confirmIngestion(database, pendingId, "create-edition", testFile);
  ...
  describe("Edge Cases and Error Handling", () => {
    it("should handle missing required fields for pending ingestion", async () => {
      ...
      await expect(ingestWork(database, testFile, { onDuplicateWork: "prompt" })).rejects.toThrow(
        "userId is required",
      );

      // Try without uploadId
      await expect(
        ingestWork(database, testFile, { userId: TEST_USER_ID, onDuplicateWork: "prompt" }),
