...
describe("Embedded Metadata Integration", () => {
  describe("Confidence Comparison", () => {
    it("should assign higher confidence to embedded metadata than typical external sources", () => {
      ...
      const embeddedRecord = (provider as any).convertToMetadataRecord(ebookMetadata, "epub");
    ...
    it("should have lower confidence than embedded for incomplete external data", () => {
      ...
      const embeddedRecord = (provider as any).convertToMetadataRecord(completeEmbedded, "epub");
  ...
  describe("Author Name Normalization", () => {
    it('should normalize "Last, First" format to "First Last"', () => {
      ...
      const ebookMetadata: EbookMetadata = {
        title: "Test Book",
        contributors: [{ name: "Tolkien, J.R.R.", roles: ["aut"], sortingKey: "Tolkien, J.R.R." }],
      };
    ...
    it("should preserve names already in correct format", () => {
      ...
      const ebookMetadata: EbookMetadata = {
        title: "Test Book",
        contributors: [{ name: "George Orwell", roles: ["aut"], sortingKey: "Orwell, George" }],
      };
  ...
  describe("Aggregation Scenarios", () => {
    it("should provide complementary data to external providers", () => {
      ...
      const embeddedRecord = (provider as any).convertToMetadataRecord(embeddedMetadata, "epub");
      ...
      const externalRecord: MetadataRecord = {
        ...
        description: "The story of Bilbo Baggins' journey to the Lonely Mountain",
      };
    ...
    it("should allow embedded metadata to override low-confidence external data", () => {
      ...
      expect(embeddedRecord.confidence).toBeGreaterThan(externalRecord.confidence);
    ...
    it("should preserve embedded cover image over external URLs", () => {
      ...
      expect(provider.getReliabilityScore(MetadataType.COVER_IMAGE)).toBeGreaterThan(0.85);
  ...
  describe("Metadata Completeness Analysis", () => {
    it("should correctly identify complete vs incomplete metadata", () => {
      ...
      const completeMetadata: EbookMetadata = {
        title: "Complete Book",
        contributors: [
          { name: "Author Name", roles: ["aut"], sortingKey: "Name, Author" },
          { name: "Publisher Name", roles: ["bkp"], sortingKey: "Name, Publisher" },
        ],
        identifiers: [{ type: "isbn", value: "978-0-123-45678-9" }],
      ...
      const incompleteMetadata: EbookMetadata = { title: "Incomplete Book", contributors: [] };

      const completeTypes = (provider as any).getAvailableMetadataTypes(completeMetadata);
      const incompleteTypes = (provider as any).getAvailableMetadataTypes(incompleteMetadata);
  ...
  describe("Real-world Scenarios", () => {
    it("should handle Calibre-managed ebook metadata", () => {
      ...
      const calibreMetadata: EbookMetadata = {
        ...
        contributors: [
          { name: "Patrick Rothfuss", roles: ["aut"], sortingKey: "Rothfuss, Patrick" },
          { name: "DAW Books", roles: ["bkp"], sortingKey: "DAW Books" },
        ],
        identifiers: [
      ...
      const record = (provider as any).convertToMetadataRecord(calibreMetadata, "epub");
      ...
      expect(record.series).toEqual({ name: "The Kingkiller Chronicle", volume: 1 });
    ...
    it("should handle minimal PDF metadata gracefully", () => {
      ...
      const pdfMetadata: EbookMetadata = { title: "Document.pdf", contributors: [] };

      const record = (provider as any).convertToMetadataRecord(pdfMetadata, "pdf");
    ...
    it("should handle indie/self-published ebooks without ISBN", () => {
      ...
      const indieMetadata: EbookMetadata = {
        ...
        contributors: [{ name: "Indie Author", roles: ["aut"], sortingKey: "Author, Indie" }],
        language: "en",
      ...
      const record = (provider as any).convertToMetadataRecord(indieMetadata, "epub");
