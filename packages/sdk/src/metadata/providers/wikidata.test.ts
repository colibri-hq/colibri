...
describe("WikiDataMetadataProvider", () => {
  ...
  const createMockBinding = (overrides: any = {}) => ({
    ...
    publisherLabel: { type: "literal", value: "Test Publisher", "xml:lang": "en" },
    languageLabel: { type: "literal", value: "English", "xml:lang": "en" },
    pages: { type: "literal", value: "300", datatype: "http://www.w3.org/2001/XMLSchema#integer" },
    description: { type: "literal", value: "A test book description", "xml:lang": "en" },
    ...overrides,
  });
  ...
  describe("Provider Priority and Integration", () => {
    describe("Priority Configuration", () => {
      ...
      it("should justify higher priority through data quality characteristics", () => {
        ...
        expect(provider.name).toBe("WikiData");
        expect(provider.getReliabilityScore(MetadataType.ISBN)).toBeGreaterThan(0.95);
        expect(provider.getReliabilityScore(MetadataType.AUTHORS)).toBeGreaterThan(0.9);
    ...
    describe("Provider Interface Compliance", () => {
      ...
      it("should have consistent method signatures with other providers", async () => {
        ...
        expect(async () => await provider.searchByTitle(titleQuery)).not.toThrow();
        expect(async () => await provider.searchByISBN("9781234567890")).not.toThrow();
        expect(async () => await provider.searchByCreator(creatorQuery)).not.toThrow();
        expect(async () => await provider.searchMultiCriteria(multiQuery)).not.toThrow();
      });
      ...
      it("should return consistent result format across all search methods", async () => {
        ...
        const titleResults = await provider.searchByTitle({ title: "Test", exactMatch: true });
        const isbnResults = await provider.searchByISBN("9781234567890");
        const authorResults = await provider.searchByCreator({ name: "Test Author", fuzzy: false });
        const multiResults = await provider.searchMultiCriteria({ title: "Test" });

        // Verify all results have consistent structure
        for (const results of [titleResults, isbnResults, authorResults, multiResults]) {
    ...
    describe("Multi-Provider Integration Scenarios", () => {
      it("should work correctly alongside other providers in priority-based selection", () => {
        ...
        const providers = [mockOpenLibraryProvider, mockGoogleBooksProvider, provider];
      ...
      it("should handle provider coordination gracefully", async () => {
        ...
        const mockBinding = createMockBinding({
          title: { type: "literal", value: "Coordination Test Book", "xml:lang": "en" },
          authorLabel: { type: "literal", value: "Coordination Author", "xml:lang": "en" },
          isbn: { type: "literal", value: "9781234567890" },
        });
    ...
    describe("CLI Integration Compatibility", () => {
      ...
      it("should provide appropriate metadata for CLI display", async () => {
        const completeBinding = createMockBinding({
          title: { type: "literal", value: "CLI Display Test", "xml:lang": "en" },
          authorLabel: { type: "literal", value: "Display Author", "xml:lang": "en" },
          isbn: { type: "literal", value: "9781234567890" },
          publishDate: {
            type: "literal",
            value: "2023-01-01",
          ...
          publisherLabel: { type: "literal", value: "Display Publisher", "xml:lang": "en" },
        });
  ...
  describe("Reliability Scores for Metadata Types", () => {
    ...
    describe("Medium Reliability Metadata Types", () => {
      it("should return appropriate scores for moderately reliable metadata types", () => {
        ...
        expect(provider.getReliabilityScore(MetadataType.DESCRIPTION)).toBe(0.75);
    ...
    describe("Lower Reliability Metadata Types", () => {
      it("should return lower scores for data types WikiData has less comprehensive coverage", () => {
        expect(provider.getReliabilityScore(MetadataType.PHYSICAL_DIMENSIONS)).toBe(0.6);
        expect(provider.getReliabilityScore(MetadataType.COVER_IMAGE)).toBe(0.3);
      });
    });
    ...
    describe("Reliability Score Ranking", () => {
      it("should rank reliability scores appropriately: ISBN > Authors > Language > Title > Publisher", () => {
        ...
        const languageScore = provider.getReliabilityScore(MetadataType.LANGUAGE);
        const titleScore = provider.getReliabilityScore(MetadataType.TITLE);
        const publisherScore = provider.getReliabilityScore(MetadataType.PUBLISHER);
    ...
    describe("WikiData Quality Characteristics", () => {
      it("should reflect WikiData strengths in structured bibliographic data", () => {
        ...
        expect(provider.getReliabilityScore(MetadataType.ISBN)).toBeGreaterThan(0.95);
        expect(provider.getReliabilityScore(MetadataType.AUTHORS)).toBeGreaterThan(0.9);
        expect(provider.getReliabilityScore(MetadataType.LANGUAGE)).toBeGreaterThan(0.9);

        // WikiData has good coverage of publication metadata
        expect(provider.getReliabilityScore(MetadataType.PUBLICATION_DATE)).toBeGreaterThan(0.85);
        expect(provider.getReliabilityScore(MetadataType.PUBLISHER)).toBeGreaterThan(0.85);
      });

      it("should reflect WikiData limitations in multimedia and physical data", () => {
        // WikiData has limited multimedia content
        expect(provider.getReliabilityScore(MetadataType.COVER_IMAGE)).toBeLessThan(0.5);

        // Physical dimensions are less commonly available
        expect(provider.getReliabilityScore(MetadataType.PHYSICAL_DIMENSIONS)).toBeLessThan(0.7);
      });
    });
  });
  ...
  describe("Title Search", () => {
    it("should search by title and return metadata records", async () => {
      ...
      const results = await provider.searchByTitle({ title: "The Great Gatsby", exactMatch: true });
    ...
    it("should handle title search with exact matching for performance", async () => {
      ...
      const results = await provider.searchByTitle({ title: "Test", exactMatch: false });
    ...
    it("should handle empty results gracefully", async () => {
      ...
      const results = await provider.searchByTitle({ title: "Nonexistent Book", exactMatch: true });
  ...
  describe("Author Search", () => {
    it("should search by author and return metadata records", async () => {
      ...
      const results = await provider.searchByCreator({ name: "F. Scott Fitzgerald", fuzzy: true });

      expect(results).toHaveLength(1);
      expect(results[0].authors).toEqual(["F. Scott Fitzgerald"]);
      // The implementation uses exact matching for performance optimization
      expect(mockFetch).toHaveBeenCalledWith(
        expect.stringContaining("%3Fauthor%20rdfs%3Alabel%20%22F.%20Scott%20Fitzgerald%22%40en"),
        expect.any(Object),
      );
    });
  });
  describe("Multi-Criteria Search", () => {
    ...
    it("should fall back to title search when no ISBN", async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        expect.stringContaining("%3Fbook%20rdfs%3Alabel%20%22Test%20Book%22%40en"),
        expect.any(Object),
      );
    });
  });
  ...
  describe("Metadata Extraction and Processing", () => {
    describe("Comprehensive Metadata Field Extraction", () => {
      it("should extract all core metadata fields correctly", async () => {
        const completeBinding = createMockBinding({
          title: { type: "literal", value: "The Complete Guide to Everything", "xml:lang": "en" },
          authorLabel: { type: "literal", value: "Jane Smith", "xml:lang": "en" },
          isbn: { type: "literal", value: "9781234567890" },
          publishDate: {
            type: "literal",
          ...
          publisherLabel: { type: "literal", value: "Academic Press", "xml:lang": "en" },
          languageLabel: { type: "literal", value: "English", "xml:lang": "en" },
          pages: {
            type: "literal",
            value: "450",
          ...
          description: {
            type: "literal",
            value: "A comprehensive guide covering all aspects of the subject matter.",
            "xml:lang": "en",
          },
        });
        ...
        expect(record.isbn).toEqual(["9781234567890"]);
        expect(record.publicationDate).toEqual(new Date("2023-06-15T00:00:00Z"));
      ...
      it("should extract optional fields when available", async () => {
        const bindingWithOptionals = createMockBinding({
          title: { type: "literal", value: "Advanced Topics in Science", "xml:lang": "en" },
          authorLabel: { type: "literal", value: "Dr. Robert Johnson", "xml:lang": "en" },
          ...
          publisherLabel: { type: "literal", value: "Science Publications", "xml:lang": "en" },
          languageLabel: { type: "literal", value: "English", "xml:lang": "en" },
          pages: {
            type: "literal",
            value: "320",
        ...
        expect(record.description).toBe("An in-depth exploration of advanced scientific concepts.");

        // Verify provider-specific data is included
        expect(record.providerData).toBeDefined();
        expect(record.providerData.wikidataUri).toBe("http://www.wikidata.org/entity/Q123456");
      ...
      it("should handle missing fields gracefully without errors", async () => {
        const minimalBinding = {
          book: { type: "uri", value: "http://www.wikidata.org/entity/Q789012" },
          title: { type: "literal", value: "Minimal Book Record", "xml:lang": "en" },
        ...
        expect(record.providerData).toBeDefined();
        expect(record.providerData.wikidataUri).toBe("http://www.wikidata.org/entity/Q789012");
      ...
      it("should handle malformed or invalid field values gracefully", async () => {
        const malformedBinding = createMockBinding({
          title: { type: "literal", value: "Book with Invalid Data", "xml:lang": "en" },
          authorLabel: { type: "literal", value: "Valid Author", "xml:lang": "en" },
      ...
      it("should preserve WikiData entity information in provider data", async () => {
        const binding = createMockBinding({
          book: { type: "uri", value: "http://www.wikidata.org/entity/Q987654321" },
          title: { type: "literal", value: "Entity Information Test", "xml:lang": "en" },
        });
        ...
        expect(record.providerData).toBeDefined();
        expect(record.providerData.wikidataUri).toBe("http://www.wikidata.org/entity/Q987654321");
    ...
    describe("Multiple Authors and ISBNs Handling", () => {
      it("should handle multiple authors for the same book", async () => {
        const mockBindings = [
          createMockBinding({
            book: { type: "uri", value: "http://www.wikidata.org/entity/Q123456" },
            title: { type: "literal", value: "Collaborative Work", "xml:lang": "en" },
        ...
        expect(results).toHaveLength(1);
        expect(results[0].authors).toEqual(["Author One", "Author Two", "Author Three"]);
      ...
      it("should handle multiple ISBNs for the same book", async () => {
        const mockBindings = [
          createMockBinding({
            book: { type: "uri", value: "http://www.wikidata.org/entity/Q654321" },
            title: { type: "literal", value: "Multi-Format Book", "xml:lang": "en" },
        ...
        expect(results).toHaveLength(1);
        expect(results[0].isbn).toEqual(["9781234567890", "9780987654321", "9785555555555"]);
      ...
      it("should handle books with both multiple authors and multiple ISBNs", async () => {
        const mockBindings = [
          createMockBinding({
            book: { type: "uri", value: "http://www.wikidata.org/entity/Q111222" },
            title: { type: "literal", value: "Complex Book Record", "xml:lang": "en" },
            authorLabel: { type: "literal", value: "First Author", "xml:lang": "en" },
          ...
          createMockBinding({
            book: { type: "uri", value: "http://www.wikidata.org/entity/Q111222" },
            title: { type: "literal", value: "Complex Book Record", "xml:lang": "en" },
            authorLabel: { type: "literal", value: "Second Author", "xml:lang": "en" },
            isbn: { type: "literal", value: "9781111111111" },
          }),
          createMockBinding({
            book: { type: "uri", value: "http://www.wikidata.org/entity/Q111222" },
      ...
      it("should deduplicate authors and ISBNs correctly", async () => {
        const mockBindings = [
          createMockBinding({
            book: { type: "uri", value: "http://www.wikidata.org/entity/Q333444" },
            title: { type: "literal", value: "Duplicate Test Book", "xml:lang": "en" },
            authorLabel: { type: "literal", value: "Duplicate Author", "xml:lang": "en" },
            isbn: { type: "literal", value: "9783333333333" },
          }),
          createMockBinding({
            book: { type: "uri", value: "http://www.wikidata.org/entity/Q333444" },
            title: { type: "literal", value: "Duplicate Test Book", "xml:lang": "en" },
            authorLabel: { type: "literal", value: "Duplicate Author", "xml:lang": "en" }, // Same author
            isbn: { type: "literal", value: "9783333333333" }, // Same ISBN
          }),
          createMockBinding({
            book: { type: "uri", value: "http://www.wikidata.org/entity/Q333444" },
            title: { type: "literal", value: "Duplicate Test Book", "xml:lang": "en" },
            authorLabel: { type: "literal", value: "Unique Author", "xml:lang": "en" },
        ...
        expect(results[0].authors).toEqual(["Duplicate Author", "Unique Author"]);
      ...
      it("should handle empty or undefined author and ISBN values", async () => {
        const mockBindings = [
          createMockBinding({
            book: { type: "uri", value: "http://www.wikidata.org/entity/Q555666" },
            title: { type: "literal", value: "Sparse Data Book", "xml:lang": "en" },
            authorLabel: { type: "literal", value: "Valid Author", "xml:lang": "en" },
          ...
          createMockBinding({
            book: { type: "uri", value: "http://www.wikidata.org/entity/Q555666" },
            title: { type: "literal", value: "Sparse Data Book", "xml:lang": "en" },
            authorLabel: { type: "literal", value: "Another Author", "xml:lang": "en" },
    ...
    describe("Publication Date Parsing", () => {
      it("should parse full ISO date format correctly", async () => {
        ...
        const results = await provider.searchByTitle({ title: "Full Date Book", exactMatch: true });

        expect(results).toHaveLength(1);
        expect(results[0].publicationDate).toEqual(new Date("2020-05-15T00:00:00Z"));
      });
      ...
      it("should parse date-only format correctly", async () => {
        ...
        const results = await provider.searchByTitle({ title: "Date Only Book", exactMatch: true });
      ...
      it("should parse year-only format correctly", async () => {
        ...
        const results = await provider.searchByTitle({ title: "Year Only Book", exactMatch: true });
      ...
      it("should handle various year formats", async () => {
        ...
        for (const testCase of testCases) {
          const mockBinding = createMockBinding({
            title: { type: "literal", value: `Book from ${testCase.input}`, "xml:lang": "en" },
            publishDate: {
              type: "literal",
      ...
      it("should handle invalid date formats gracefully", { timeout: 10000 }, async () => {
        ...
        for (const invalidDate of invalidDateCases) {
          const mockBinding = createMockBinding({
            title: { type: "literal", value: "Invalid Date Book", "xml:lang": "en" },
            publishDate: {
              type: "literal",
              value: invalidDate,
              datatype: "http://www.w3.org/2001/XMLSchema#date",
      ...
      it("should handle missing publication date gracefully", async () => {
        ...
        const results = await provider.searchByTitle({ title: "No Date Book", exactMatch: true });
      ...
      it("should create valid Date objects for valid dates", async () => {
        const validDates = [
          { input: "2020-01-01T00:00:00Z", expected: new Date("2020-01-01T00:00:00Z") },
          { input: "2021-06-15", expected: new Date("2021-06-15") },
          { input: "2022-12-31T23:59:59Z", expected: new Date("2022-12-31T23:59:59Z") },
        ];

        for (const dateCase of validDates) {
          const mockBinding = createMockBinding({
            title: { type: "literal", value: "Valid Date Book", "xml:lang": "en" },
            publishDate: {
              type: "literal",
    ...
    describe("Language Code Mapping", () => {
      it("should map common language labels to ISO codes", async () => {
        ...
        const englishBinding = createMockBinding({
          title: { type: "literal", value: "Book in English", "xml:lang": "en" },
          languageLabel: { type: "literal", value: "English", "xml:lang": "en" },
        });
      ...
      it("should map Spanish language label to ISO code", async () => {
        const spanishBinding = createMockBinding({
          title: { type: "literal", value: "Book in Spanish", "xml:lang": "en" },
          languageLabel: { type: "literal", value: "Spanish", "xml:lang": "en" },
        });
      ...
      it("should map additional language labels to ISO codes", async () => {
        ...
        const italianBinding = createMockBinding({
          title: { type: "literal", value: "Book in Italian", "xml:lang": "en" },
          languageLabel: { type: "literal", value: "Italian", "xml:lang": "en" },
        });
        ...
        const portugueseBinding = createMockBinding({
          title: { type: "literal", value: "Book in Portuguese", "xml:lang": "en" },
          languageLabel: { type: "literal", value: "Portuguese", "xml:lang": "en" },
        });
      ...
      it("should handle unknown languages gracefully", async () => {
        ...
        const klingonBinding = createMockBinding({
          title: { type: "literal", value: "Book in Klingon", "xml:lang": "en" },
      ...
      it("should handle fictional languages gracefully", async () => {
        ...
        const dothrakiBinding = createMockBinding({
          title: { type: "literal", value: "Book in Dothraki", "xml:lang": "en" },
          languageLabel: { type: "literal", value: "Dothraki", "xml:lang": "en" },
        });
      ...
      it("should preserve original language if mapping fails", async () => {
        ...
        const singleCharBinding = createMockBinding({
          title: { type: "literal", value: "Single Char Book", "xml:lang": "en" },
      ...
      it("should handle edge cases in language mapping", async () => {
        ...
        const threeCharBinding = createMockBinding({
          title: { type: "literal", value: "Three Char Book", "xml:lang": "en" },
        ...
        const upperBinding = createMockBinding({
          title: { type: "literal", value: "Upper Case Book", "xml:lang": "en" },
      ...
      it("should handle missing language labels gracefully", async () => {
        const mockBinding = createMockBinding({
          title: { type: "literal", value: "No Language Book", "xml:lang": "en" },
      ...
      it("should handle empty language labels gracefully", async () => {
        const mockBinding = createMockBinding({
          title: { type: "literal", value: "Empty Language Book", "xml:lang": "en" },
      ...
      it("should handle case-insensitive language mapping", async () => {
        ...
        for (const mapping of caseMappings) {
          const mockBinding = createMockBinding({
            title: { type: "literal", value: "Case Test Book", "xml:lang": "en" },
            languageLabel: { type: "literal", value: mapping.input, "xml:lang": "en" },
          });
  ...
  describe("Language Code Mapping (Task 3.4)", () => {
    describe("Common Language Labels to ISO Codes", () => {
      it("should map English to en", async () => {
        ...
        const results = await provider.searchByTitle({ title: "English Book", exactMatch: true });
      ...
      it("should map German to de", async () => {
        ...
        const results = await provider.searchByTitle({ title: "German Book", exactMatch: true });
      ...
      it("should map French to fr", async () => {
        ...
        const results = await provider.searchByTitle({ title: "French Book", exactMatch: true });
      ...
      it("should map Spanish to es", async () => {
        ...
        const results = await provider.searchByTitle({ title: "Spanish Book", exactMatch: true });
      ...
      it("should map Italian to it", async () => {
        ...
        const results = await provider.searchByTitle({ title: "Italian Book", exactMatch: true });
      ...
      it("should map Portuguese to pt", async () => {
        const mockBinding = createMockBinding({
          title: { type: "literal", value: "Portuguese Book", "xml:lang": "en" },
          languageLabel: { type: "literal", value: "Portuguese", "xml:lang": "en" },
        });
      ...
      it("should map Dutch to nl", async () => {
        ...
        const results = await provider.searchByTitle({ title: "Dutch Book", exactMatch: true });
      ...
      it("should map Russian to ru", async () => {
        const mockBinding = createMockBinding({
          ...
          languageLabel: { type: "literal", value: "Russian", "xml:lang": "en" },
        });
        ...
        const results = await provider.searchByTitle({ title: "Russian Book", exactMatch: true });
      ...
      it("should map Japanese to ja", async () => {
        const mockBinding = createMockBinding({
          ...
          languageLabel: { type: "literal", value: "Japanese", "xml:lang": "en" },
        });
        ...
        const results = await provider.searchByTitle({ title: "Japanese Book", exactMatch: true });
      ...
      it("should map Chinese to zh", async () => {
        const mockBinding = createMockBinding({
          ...
          languageLabel: { type: "literal", value: "Chinese", "xml:lang": "en" },
        });
        ...
        const results = await provider.searchByTitle({ title: "Chinese Book", exactMatch: true });
      ...
      it("should map Korean to ko", async () => {
        ...
        const results = await provider.searchByTitle({ title: "Korean Book", exactMatch: true });
      ...
      it("should map Arabic to ar", async () => {
        ...
        const results = await provider.searchByTitle({ title: "Arabic Book", exactMatch: true });
    ...
    describe("Unknown Languages Handling", () => {
      it("should handle unknown languages gracefully by using first 2 characters", async () => {
        ...
        const results = await provider.searchByTitle({ title: "Klingon Book", exactMatch: true });
      ...
      it("should handle fictional languages gracefully", async () => {
        ...
        const results = await provider.searchByTitle({ title: "Elvish Book", exactMatch: true });
      ...
      it("should handle constructed languages gracefully", async () => {
        ...
        const results = await provider.searchByTitle({ title: "Dothraki Book", exactMatch: true });
      ...
      it("should handle very short unknown languages", async () => {
        ...
        const mockBinding = createMockBinding({
          title: { type: "literal", value: "X Language Book", "xml:lang": "en" },
          languageLabel: { type: "literal", value: "X", "xml:lang": "en" },
        });
      ...
      it("should handle two character unknown languages", async () => {
        ...
        const mockBinding = createMockBinding({
          title: { type: "literal", value: "AB Language Book", "xml:lang": "en" },
          languageLabel: { type: "literal", value: "AB", "xml:lang": "en" },
        });
      ...
      it("should handle uppercase unknown languages by converting to lowercase", async () => {
        ...
        const mockBinding = createMockBinding({
          ...
          languageLabel: { type: "literal", value: "UNKNOWN", "xml:lang": "en" },
        });
        ...
        const results = await provider.searchByTitle({ title: "UNKNOWN Book", exactMatch: true });
    ...
    describe("Original Language Preservation", () => {
      it("should preserve original language when mapping fails for single character", async () => {
        const mockBinding = createMockBinding({
          title: { type: "literal", value: "Single Char Language Book", "xml:lang": "en" },
      ...
      it("should preserve original language when mapping fails for two characters", async () => {
        const mockBinding = createMockBinding({
          title: { type: "literal", value: "Two Char Language Book", "xml:lang": "en" },
      ...
      it("should use first 2 characters when mapping fails for longer strings", async () => {
        ...
        const mockBinding = createMockBinding({
          title: { type: "literal", value: "UnknownLanguage Book", "xml:lang": "en" },
          languageLabel: { type: "literal", value: "UnknownLanguage", "xml:lang": "en" },
        });
      ...
      it("should handle null language labels gracefully", async () => {
        const mockBinding = createMockBinding({
          title: { type: "literal", value: "Null Language Book", "xml:lang": "en" },
  ...
  describe("Language Mapping", () => {
    it("should map common language labels to ISO codes", async () => {
      ...
      const results = await provider.searchByTitle({ title: "Test Book", exactMatch: true });
  ...
  describe("Confidence Calculation Algorithm", () => {
    describe("Base Confidence by Search Type", () => {
      it("should assign 90%+ base confidence to ISBN searches", async () => {
        ...
        const minimalBinding = {
          book: { type: "uri", value: "http://www.wikidata.org/entity/Q123456" },
          title: { type: "literal", value: "ISBN Search Book", "xml:lang": "en" },
          isbn: { type: "literal", value: "9781234567890" },
        };
      ...
      it("should assign appropriate base confidence to title searches", async () => {
        ...
        const minimalBinding = {
          book: { type: "uri", value: "http://www.wikidata.org/entity/Q123456" },
          title: { type: "literal", value: "Title Search Book", "xml:lang": "en" },
        };
      ...
      it("should assign appropriate base confidence to author searches", async () => {
        ...
        const minimalBinding = {
          book: { type: "uri", value: "http://www.wikidata.org/entity/Q123456" },
          title: { type: "literal", value: "Author Search Book", "xml:lang": "en" },
          authorLabel: { type: "literal", value: "Test Author", "xml:lang": "en" },
        };
        ...
        const results = await provider.searchByCreator({ name: "Test Author", fuzzy: false });
      ...
      it("should rank confidence scores: ISBN > title > author", async () => {
        const baseBinding = {
          book: { type: "uri", value: "http://www.wikidata.org/entity/Q123456" },
          title: { type: "literal", value: "Comparison Book", "xml:lang": "en" },
          authorLabel: { type: "literal", value: "Test Author", "xml:lang": "en" },
          isbn: { type: "literal", value: "9781234567890" },
        };
        ...
        expect(isbnResults[0].confidence).toBeGreaterThan(titleResults[0].confidence);
        expect(titleResults[0].confidence).toBeGreaterThan(authorResults[0].confidence);
      });
    });
    ...
    describe("Data Completeness Boosts", () => {
      it("should boost confidence for complete metadata records", async () => {
        const completeBinding = createMockBinding({
          ...
          authorLabel: { type: "literal", value: "Complete Author", "xml:lang": "en" },
          isbn: { type: "literal", value: "9781234567890" },
          publishDate: {
            type: "literal",
            value: "2023-01-01",
          ...
          publisherLabel: { type: "literal", value: "Complete Publisher", "xml:lang": "en" },
          languageLabel: { type: "literal", value: "English", "xml:lang": "en" },
          pages: {
            type: "literal",
            value: "300",
          ...
          description: { type: "literal", value: "A complete book description", "xml:lang": "en" },
        });
        ...
        const results = await provider.searchByTitle({ title: "Complete Book", exactMatch: true });
        ...
        const testCases = [
          {
            ...
            binding: {
              book: { type: "uri", value: "http://www.wikidata.org/entity/Q123456" },
              title: { type: "literal", value: "Title Only Book", "xml:lang": "en" },
            },
          },
          {
            name: "title + author",
            binding: {
              book: { type: "uri", value: "http://www.wikidata.org/entity/Q123456" },
              title: { type: "literal", value: "Title Author Book", "xml:lang": "en" },
              authorLabel: { type: "literal", value: "Test Author", "xml:lang": "en" },
            },
          },
          {
            ...
            binding: {
              book: { type: "uri", value: "http://www.wikidata.org/entity/Q123456" },
              title: { type: "literal", value: "Title Author ISBN Book", "xml:lang": "en" },
              authorLabel: { type: "literal", value: "Test Author", "xml:lang": "en" },
              isbn: { type: "literal", value: "9781234567890" },
            },
          },
      ...
      it("should cap confidence at 0.92 maximum", async () => {
        ...
        const maxBinding = createMockBinding({
          title: { type: "literal", value: "Max Confidence Book", "xml:lang": "en" },
          authorLabel: { type: "literal", value: "Max Author", "xml:lang": "en" },
          isbn: { type: "literal", value: "9781234567890" },
          publishDate: {
            type: "literal",
            value: "2023-01-01",
          ...
          publisherLabel: { type: "literal", value: "Max Publisher", "xml:lang": "en" },
          languageLabel: { type: "literal", value: "English", "xml:lang": "en" },
          pages: {
            type: "literal",
            value: "500",
      ...
      it("should handle missing completeness fields gracefully", async () => {
        ...
        const results = await provider.searchByTitle({ title: "Sparse Book", exactMatch: true });
    ...
    describe("WikiData Quality Bonus", () => {
      it("should apply WikiData quality bonus to all searches", async () => {
        const testBinding = {
          book: { type: "uri", value: "http://www.wikidata.org/entity/Q123456" },
          title: { type: "literal", value: "Quality Bonus Book", "xml:lang": "en" },
        };
    ...
    describe("Confidence Precision", () => {
      it("should round confidence to 2 decimal places", async () => {
        const testBinding = {
          book: { type: "uri", value: "http://www.wikidata.org/entity/Q123456" },
          title: { type: "literal", value: "Precision Test Book", "xml:lang": "en" },
          authorLabel: { type: "literal", value: "Precision Author", "xml:lang": "en" },
        };
  ...
  describe("Performance Tests", () => {
    describe("Query Performance", () => {
      ...
      it("should use exact matching for optimal performance", async () => {
        ...
        await provider.searchByTitle({ title: "Exact Title", exactMatch: true });

        // Verify exact matching is used (not CONTAINS or fuzzy matching)
        expect(mockFetch).toHaveBeenCalledWith(
          expect.stringContaining("%3Fbook%20rdfs%3Alabel%20%22Exact%20Title%22%40en"),
          expect.any(Object),
        );
      ...
      it("should handle performance edge cases with complex queries", async () => {
        ...
        const results = await provider.searchByTitle({ title: longTitle, exactMatch: true });
    ...
    describe("Rate Limiting Tests", () => {
      it("should enforce 1-second delay between requests", async () => {
        ...
        await provider.searchByTitle({ title: "Second Book", exactMatch: true });
    ...
    describe("Retry Logic and Error Handling Tests", () => {
      ...
      it("should retry on network errors with exponential backoff", async () => {
        ...
        const results = await retryProvider.searchByTitle({ title: "Test Book", exactMatch: true });
      ...
      it("should retry on 5xx HTTP errors but not 4xx errors", async () => {
        ...
        const retryProvider4xx = new WikiDataMetadataProvider(retryMockFetch4xx);
