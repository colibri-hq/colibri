...
export async function createImage(
  ...
  file = new File([file], `images/${checksum.toString("base64url")}/${name}.${ext}`, {
    type: file.type ?? mime,
    lastModified: file.lastModified,
  });

  const storage = await createStorage(database);
  const fileUrl = await writeFile(storage, file, { blurhash });
    ...
    return database
      ...
      .onConflict((oc) =>
        oc
          .column("checksum")
          .doUpdateSet((eb) => ({
            metadata: mergeJson(eb.ref("image.metadata"), eb.ref("excluded.metadata")),
...
export function findImageByChecksum(
  ...
  return database.selectFrom(table).where("checksum", "=", buffer).selectAll().executeTakeFirst();
}
