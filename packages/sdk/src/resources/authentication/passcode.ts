...
export async function verifyPasscode(client: Database, email: string, code: string) {
  const result = await client
    .deleteFrom(table)
    .where((expression) => expression.and({ email, code }).and("expires_at", ">", new Date()))
...
export function createPasscode(
  ...
  return client
    ...
    .onConflict((conflict) =>
      conflict.column("user_id").doUpdateSet({ expires_at: new Date(Date.now() + ttl), code }),
    )
    .returning("code")
    .executeTakeFirstOrThrow();
}
