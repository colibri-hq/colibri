...
export class Epub {
  ...
  async loadDocument({ href, mediaType }: ManifestItem) {
    const source = await this.loadText(href);

    return this.#parser.parseFromString(source, mediaType);
  }
...
function firstValue<T>(value: T | T[] | undefined) {
  if (typeof value === "undefined") {
    return undefined;
  }

  return wrapArray(value).at(0);
}
