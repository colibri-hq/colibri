...
${languages
  .map(
    (l) =>
      `  { iso3: "${l.iso3}", iso1: ${l.iso1 ? `"${l.iso1}"` : "null"}, type: "${l.type}" as LanguageType, name: "${escapeTS(l.name)}" }`,
  )
  .join(",\n")}
]);
...
`;
  ...
  console.log(`Generated languages.ts (${languages.length} languages, ${iso1Entries.length} ISO 639-1 codes, ${nameEntries.length} name entries)`);
...
${languages
  .map(
    (l) =>
      `('${l.iso3}', ${l.iso1 ? `'${l.iso1}'` : "null"}, '${l.type}', '${escapeSQL(l.name)}')`,
  )
  .join(",\n")};
`;
