<script lang="ts">
  ...
  const { open: openSearch } = getSearchContext();
  ...
  function getSlug(item: Page | Directory): string {
    return item.slug;
  }
  ...
  function isActive(item: Page | Directory): boolean {
    const slug = getSlug(item);
    return page.url.pathname === slug || page.url.pathname.startsWith(slug + '/');
  }

  function isBlogActive(): boolean {
    return page.url.pathname === '/blog' || page.url.pathname.startsWith('/blog/');
  }
</script>

<nav
  class="bg-slate-900/80 backdrop-blur-xl backdrop-saturate-200 text-white shadow-md sticky w-full top-0 z-10"
>
  <div class="container mx-auto px-4">
    <div class="h-16 grid items-center grid-cols-[auto_1fr_auto] gap-4">

      <!-- region Logo -->
      <div class="flex items-center">
        <a href={resolve("/")}
           class="flex items-center gap-2 outline-hidden focus-visible:text-blue-400 transition-colors duration-200">
          <div class="size-8">
      ...
      <div>
        <SearchTrigger class="hidden lg:flex" onclick={openSearch} />
      </div>

      <!-- region Desktop Menu -->
      <nav class="hidden md:flex items-center gap-2">
        <ul class="contents select-none">
          {#each items as item (item.slug)}
            <li class="contents">
              <a
                href={getSlug(item)}
                class="py-2 px-3 font-medium text-sm transition duration-200 rounded-lg outline-hidden
                focus-visible:ring-2 focus-visible:ring-blue-400 dark:focus-visible:ring-blue-500
                {isActive(item) ? 'text-blue-400 bg-slate-800' : 'text-slate-300 hover:text-white hover:bg-slate-800'}"
              >
                {getTitle(item)}
              </a>
            </li>
  ...
  {#if isMenuOpen}
    <div
      class="md:hidden bg-slate-800/95 backdrop-blur-lg border-t border-slate-700"
      ...
      <div class="px-2 pt-2 pb-3 space-y-1">
        <ul class="select-none">
          {#each items as item, i (item.slug)}
            <li
              in:fade={{ delay: 50 + i * 30, duration: 150 }}
              out:fade={{ duration: 100 }}
            >
              <a
                href={getSlug(item)}
                class="block px-3 py-2 rounded-md text-base font-medium transition-colors duration-150 {isActive(item) ? 'bg-slate-700 text-white' : 'text-slate-300 hover:bg-slate-700 hover:text-white'}"
              >
                {getTitle(item)}
              </a>
            </li>
          ...
          <li
            in:fade={{ delay: 50 + items.length * 30, duration: 150 }}
            out:fade={{ duration: 100 }}
          >
            <a
              href={resolve("/(blog)/blog")}
              class="block px-3 py-2 rounded-md text-base font-medium transition-colors duration-150 {isBlogActive() ? 'bg-slate-700 text-white' : 'text-slate-300 hover:bg-slate-700 hover:text-white'}"
            >
              Blog
            </a>
          </li>
          ...
          <li
            class="flex items-center gap-3 px-3 py-2 mt-2"
            in:fade={{ delay: 50 + (items.length + 2) * 30, duration: 150 }}
            out:fade={{ duration: 100 }}
          >
            <span class="text-base font-medium text-slate-300">Theme</span>
            <ThemeToggle />
          </li>
        </ul>
      </div>
