...
type HateoasLinks = {
  ...
  siblings?: { previous?: HateoasLink; next?: HateoasLink };
...
function serializeChildren(
...
): (ChildPageSummary | ChildDirectorySummary)[] {
  return children.map((child) => {
    ...
    return { type: "page" as const, slug: "", title: "Unknown" };
  });
}
...
export const entries: EntryGenerator = () => {
  ...
  const pageEntries = Array.from(pages.keys())
    ...
    .map((slug) => ({ path: `${slug.replace(/^\//, "")}.json` }));
...
export const GET: RequestHandler = async ({ params, url }) => {
  ...
  if (!page && directory) {
    ...
    const childrenForLinks = children.map((c) => ({ slug: c.slug, title: c.title, type: c.type }));
  ...
  let childrenForLinks:
    | Array<{ slug: string; title: string; type: "page" | "directory" }>
    | undefined;

  if (page.isIndexPage && directory) {
    children = serializeChildren(directory.children);
    childrenForLinks = children.map((c) => ({ slug: c.slug, title: c.title, type: c.type }));
  }
