<script lang="ts">
  ...
  import { BookOpenIcon, HomeIcon, NewspaperIcon } from '@lucide/svelte';
  ...
  const suggestedLinks = [
    { href: resolve('/'), label: 'Home', icon: HomeIcon, description: 'Back to the homepage' },
    {
      href: resolve('/getting-started'),
      label: 'Getting Started',
      icon: BookOpenIcon,
      description: 'Learn how to use Colibri',
    },
    { href: resolve('/(blog)/blog'), label: 'Blog', icon: NewspaperIcon, description: 'Read our latest posts' },
  ];
</script>

<svelte:head>
  <title>{status} - {isNotFound ? 'Page Not Found' : 'Error'} | Colibri Docs</title>
</svelte:head>
...
<div class="min-h-[calc(100vh-4rem)] flex items-center justify-center px-4 py-16">
  <div class="text-center max-w-2xl mx-auto">
    ...
    <div class="mb-8 relative">
      <div class="size-32 mx-auto text-slate-300 dark:text-slate-600 opacity-50">
        <svg viewBox="0 0 512 512" fill="currentColor" xmlns="http://www.w3.org/2000/svg" class="animate-pulse">
          <path
      ...
      <div class="absolute inset-0 flex items-center justify-center">
        <span class="text-6xl font-bold text-slate-400 dark:text-slate-500">{status}</span>
      </div>
    </div>
    ...
    <p class="text-lg text-gray-600 dark:text-gray-400 mb-8 max-w-md mx-auto">
      {#if isNotFound}
        ...
        Try searching or check out the links below.
