...
<div
  {...rest}
  aria-hidden="true"
  class="w-full backdrop-blur-md transition will-change-auto {className}"
>
  <!-- On top of the blurhash or gradient area, we render some noise, making for a neat visual effect -->
  <GrainOverlay
    class="absolute {blurhash ? 'opacity-35 dark:opacity-90' : ''}"
  />

  {#if blurhash}
    <BlurhashPanel class="h-full w-full" {blurhash} />
  {:else}
    <div class="absolute inset-0 bg-linear-to-r from-cyan-500 to-blue-500 opacity-60"></div>
  {/if}
</div>
