...
<Portal>
  <dialog
    class="transparency-reduce:backdrop:backdrop-blur-0 m-auto max-h-[80vh] max-w-[calc(100%_-_4rem)] transform-gpu rounded-3xl
    ...
    bind:this={ref} onClickOutside={close} use:clickOutside>
    <div
      class=""

    >
      <header
        class="sticky top-0 z-10 flex min-w-72 items-center justify-between border-b border-gray-300
      bg-white/80 pt-2 pr-2 pb-1 pl-4 backdrop-blur dark:border-b-gray-900 dark:bg-black/70"
      >
        <div>
          {@render header?.()}
        </div>
