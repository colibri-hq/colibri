...
type BookMetadataOutput = WorkerMessage<
  "search",
  { amount: number; books: Array<Record<string, unknown>> }
>;

export type BookMetadataWorker = WebWorker<BookMetadataInput, BookMetadataOutput>;

console.log("Book Metadata Worker started");

onmessage = async function ({ data: { payload } }: MessageEvent<BookMetadataInput>) {
