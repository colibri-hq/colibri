...
alter table only public.creator
    add constraint creator_updated_by_fkey foreign key (updated_by) references authentication."user" (id)
...
alter table public.creator_comment
    alter column creator_id add generated by default as identity (
        sequence name public.creator_comment_creator_id_seq
        start with 1
        increment by 1
        no minvalue
        no maxvalue
...
alter table only public.contribution
    add constraint contribution_creator_id_fkey foreign key (creator_id) references public.creator (id)
        on update cascade on delete cascade;
alter table only public.cover_creator
    add constraint cover_creator_creator_id_fkey foreign key (creator_id) references public.creator (id)
        on update cascade on delete cascade;
