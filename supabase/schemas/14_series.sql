...
alter table public.series
    owner to postgres;
comment on column public.series.name is 'Name of the series of works.';
alter table public.series
...
alter table public.series_entry
    alter column work_id add generated by default as identity (
        sequence name public.series_entry_work_id_seq
        start with 1
        increment by 1
        no minvalue
        no maxvalue
...
alter table only public.series_entry
    add constraint series_entry_work_id_fkey foreign key (work_id) references public.work (id)
        on update cascade on delete cascade;
alter table only public.series_entry
