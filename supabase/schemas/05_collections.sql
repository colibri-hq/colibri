...
comment on column public.collection.icon is 'Optional icon URN to represent this collection. Format: urn:colibri:icon:<type>:<value>';
...
comment on column public.collection.shared is 'Collection visibility: NULL = shared (all instance users), false = private (owner only), true = public (unauthenticated access).';
comment on column public.collection.like_count is 'Denormalized count of likes from user_collection_favorite table.';
alter table public.collection
